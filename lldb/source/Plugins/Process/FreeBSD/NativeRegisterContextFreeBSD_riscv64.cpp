...
// We need translation between RegisterInfoPosix_riscv64 and reg.h
...
void NativeRegisterContextFreeBSD_riscv64::FreeBSDToPOSIXGPR(
  ...
  // POSIX layout: gpr[0] = PC, gpr[1] = x1/ra, gpr[2] = x2/sp, etc.
  // FreeBSD layout: separate fields (sepc, ra, sp, gp, tp, t[], s[], a[])
...
void NativeRegisterContextFreeBSD_riscv64::POSIXToFreeBSDGPR(
  ...
  // Note: FreeBSD also has sstatus field, but we don't set it here
  // as it's typically managed by the kernel
}
...
Status NativeRegisterContextFreeBSD_riscv64::ReadAllRegisterValues(
  ...
  // Convert FreeBSD format to POSIX format (SINGLE CALL!)
...
Status NativeRegisterContextFreeBSD_riscv64::WriteAllRegisterValues(
  ...
  // Convert POSIX format to FreeBSD format (SINGLE CALL!)
